      ...
      <header className="w-full bg-white/95 shadow-lg sticky top-0 z-50 border-b border-zinc-200">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 py-3 gap-4">
          ...
          <nav className="hidden lg:flex gap-6 ml-8 items-center">
            ...
            {/* Brands dropdown */}
            <DropdownMenu>
              <DropdownMenuTrigger className="flex items-center gap-1 text-base font-medium text-gray-700 hover:text-indigo-600 transition-colors focus:outline-none">
                Brands
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" className="w-56 max-h-72 overflow-y-auto">
                <DropdownMenuLabel>Brands</DropdownMenuLabel>
                {brandLoading ? <DropdownMenuItem disabled>Loading...</DropdownMenuItem> :
                  brandError ? <DropdownMenuItem disabled className="text-red-500">{brandError}</DropdownMenuItem> :
                  brands.map((brand) => (
                    <DropdownMenuItem asChild key={brand.slug}>
                      <Link to={`/brand/${brand.slug}`} className="flex items-center gap-2">
        ...
        <div className="w-full bg-zinc-50 border-t border-zinc-200 shadow-sm">
          <div className="max-w-7xl mx-auto px-4 flex gap-4 overflow-x-auto scrollbar-thin scrollbar-thumb-zinc-200 py-2">
            {catLoading ? (
